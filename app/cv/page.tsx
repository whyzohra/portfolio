  ...
  return (
    <article className="page max-w-5xl">
      <header className="section !pt-0 border-b border-[var(--border-strong)]">
        ...
        <p className="mt-5 font-mono text-xs">{PERSONAL_INFO.taglineEngineerShort}</p>
      ...
      <CVSection title="LEADERSHIP & CAMPUS INVOLVEMENT">
        {LEADERSHIP.map((item) => (
          <p key={`${item.title}-${item.organization}`} className="border-t border-[var(--border-subtle)] py-4 text-sm">
            <strong>{item.title}</strong>
            <span className="text-[var(--text-muted)]"> — {item.organization}</span>
          </p>
        ))}
      </CVSection>
      ...
      <CVSection title="TECHNICAL SKILLS">
        {FULL_TECHNICAL_INDEX.map((s) => (
          <p key={s.category} className="border-t border-[var(--border-subtle)] py-4">
            ...
            <span className="text-[var(--text-muted)]">{s.items.join(', ')}</span>
          </p>
        ))}
      </CVSection>
      <CVSection title="CERTIFICATIONS">
        {CERTIFICATIONS.map((c) => (
          <p key={c.title} className="border-t border-[var(--border-subtle)] py-4">

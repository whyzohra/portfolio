  ...
  return (
    <div className={`project-terminal${active ? ' project-terminal--active' : ''}`} aria-label={`${title} interactive project deep dive`}>
      ...
      <div className="project-terminal__body">
        <p className="project-terminal__command"><span>zohra@portfolio:~$</span> inspect project --deep</p>
        ...
        <div className="project-terminal__stream" aria-live="polite">
          ...
          {!isTyping && visible < steps.length ? (
            <button ref={nextPrompt} type="button" className="project-terminal__next" onClick={runNext}>
              <span>zohra@portfolio:~$</span> {steps[visible].command}<i aria-hidden="true" />
              <small>CLICK OR PRESS ENTER ANYWHERE TO RUN</small>
            </button>
          ) : !isTyping ? (
            <div className="project-terminal__complete">
              <p><span>zohra@portfolio:~$</span> session --complete</p>
...
function AnimatedTerminalEntry({ step, animate, onDone }: { step: TerminalDiveStep; animate: boolean; onDone: () => void }) {
  ...
  return <section className="project-terminal__entry">
    <p className="project-terminal__command"><span>zohra@portfolio:~$</span> {step.command.slice(0, commandCount)}{commandCount < step.command.length && <i className="project-terminal__cursor" />}</p>
